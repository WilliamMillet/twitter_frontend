...
import FullWidthLoadingSpinner from "../../FullWidthLoadingSpinner/FullWidthLoadingSpinner";
...
const MutedWords = ({ setSelectedOption }) => {
  ...
  const [selectedMuteFromValue, setSelectedMuteFromValue] = useState("fromAnyone");
  const [selectedDurationValue, setSelectedDurationValue] = useState("untilYouUnmuteTheWord");
  ...
  const {
    response: getMutedWordResponse,
    error: getMutedWordError,
    loading: getMutedWordLoading,
    fetchData: getMutedWordFetchFunction,
  } = useFetchData();

  const {
    error: postMutedWordError,
    success: postMutedWordSuccess,
    fetchData: postMutedWordFetchFunction,
  } = useFetchData();

  const {
    error: deleteMutedWordError,
    fetchData: deleteMutedWordFetchFunction,
  } = useFetchData();
  ...
  useEffect(() => {
    setCommonWordError(commonWords?.includes(watchedMutedWordInput))
  }, [watchedMutedWordInput]);

  if (getMutedWordLoading) {
    return <FullWidthLoadingSpinner heightInPx={500}/>
  }
          ...
          {getMutedWordResponse?.length > 0 ? (
            <>
              ...
              <div className="muted-word-list-container">
                {getMutedWordResponse.map((word, index) => (
              ...
              <div>
              {getMutedWordError && (
                <p className="standard-input-error">{getMutedWordError}</p>
              )}
              {deleteMutedWordError && (
                <p className="standard-input-error">{deleteMutedWordError}</p>
              )}
              </div>
            </>
          ) : (
      ...
      {screenDisplayed === "add-muted-words" && (
        <>
          ...
          >
            ...
            {postMutedWordError && (
              <p className="backend-error-text">{postMutedWordError}</p>
            )}
            {postMutedWordSuccess && (
